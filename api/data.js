    ...
    try {
        ...
        if (route === 'dashboard-analitico') {
            ...
            for (var mx = 0; mx < meses3.length; mx++) {
                ...
                var rs3 = await Promise.all([
                    ...
                    // Orçamentos aprovados (vendas): 1 entrada por treatment, valor = parcela × qtd
                    client.execute({ sql: "SELECT COUNT(*) as total, COALESCE(SUM(vt),0) as valor FROM (SELECT treatment_id, MIN(valor) as vp, MAX(CASE WHEN parcelas>0 THEN parcelas ELSE 1 END) as np, MIN(valor)*MAX(CASE WHEN parcelas>0 THEN parcelas ELSE 1 END) as vt FROM pagamentos WHERE strftime('%Y-%m',data_checkout)=? AND cancelado=0 AND treatment_id IS NOT NULL AND treatment_id!='' GROUP BY treatment_id)", args: [mm] }),
        ...
        if (route === 'relatorio-profissionais') {
            ...
            var rs4 = await Promise.all([
                ...
                // Orçamentos aprovados por profissional: 1 por treatment, valor = parcela × qtd
                client.execute({ sql: "SELECT pr.nome as profissional,COUNT(DISTINCT t.treatment_id) as orcamentos,COALESCE(SUM(t.vt),0) as valor_orcamentos FROM (SELECT treatment_id, paciente_id, MIN(valor) as vp, MAX(CASE WHEN parcelas>0 THEN parcelas ELSE 1 END) as np, MIN(valor)*MAX(CASE WHEN parcelas>0 THEN parcelas ELSE 1 END) as vt, MIN(data_checkout) as checkout FROM pagamentos WHERE data_checkout>=? AND cancelado=0 AND treatment_id IS NOT NULL AND treatment_id!='' GROUP BY treatment_id) t INNER JOIN (SELECT DISTINCT paciente_id, profissional_id FROM agendamentos WHERE profissional_id IS NOT NULL) a2 ON t.paciente_id=a2.paciente_id INNER JOIN profissionais pr ON pr.id=a2.profissional_id GROUP BY pr.id ORDER BY valor_orcamentos DESC", args: [desde] }),
