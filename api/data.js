    ...
    try {
        ...
        if (route === 'dashboard-analitico') {
            ...
            for (var mx = 0; mx < meses3.length; mx++) {
                ...
                var rs3 = await Promise.all([
                    ...
                    // Orçamentos aprovados (vendas): soma todas parcelas por treatment com checkout no mês
                    client.execute({ sql: "SELECT COUNT(*) as total, COALESCE(SUM(vt),0) as valor FROM (SELECT treatment_id, SUM(valor) as vt FROM pagamentos WHERE strftime('%Y-%m',data_checkout)=? AND cancelado=0 AND treatment_id IS NOT NULL AND treatment_id!='' GROUP BY treatment_id)", args: [mm] }),
        ...
        if (route === 'relatorio-profissionais') {
            ...
            var rs4 = await Promise.all([
                ...
                // Orçamentos aprovados por profissional: soma todas parcelas por treatment
                client.execute({ sql: "SELECT pr.nome as profissional,COUNT(DISTINCT t.treatment_id) as orcamentos,COALESCE(SUM(t.vt),0) as valor_orcamentos FROM (SELECT treatment_id, paciente_id, SUM(valor) as vt FROM pagamentos WHERE data_checkout>=? AND cancelado=0 AND treatment_id IS NOT NULL AND treatment_id!='' GROUP BY treatment_id) t INNER JOIN (SELECT DISTINCT paciente_id, profissional_id FROM agendamentos WHERE profissional_id IS NOT NULL) a2 ON t.paciente_id=a2.paciente_id INNER JOIN profissionais pr ON pr.id=a2.profissional_id GROUP BY pr.id ORDER BY valor_orcamentos DESC", args: [desde] }),
