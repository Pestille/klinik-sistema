...
// Colunas reais do banco: agendamentos(data, hora_inicio, hora_fim, categoria, status)
//                         financeiro(tipo, valor, data_pagamento, forma_pagamento, descricao)
//                         pacientes(nome, telefone, data_nascimento, ativo, created_at)
//                         profissionais(nome, especialidade, ativo)
    ...
    try {
        ...
        if (route === 'dashboard') {
            ...
            var rs = await Promise.all([
                ...
                client.execute({ sql: "SELECT COUNT(*) as total FROM agendamentos WHERE data=?", args: [hoje] }),
                client.execute({ sql: "SELECT COUNT(*) as total FROM agendamentos WHERE data LIKE ?", args: [mesStr + '%'] }),
                client.execute({ sql: "SELECT COALESCE(SUM(valor),0) as total FROM financeiro WHERE tipo='entrada' AND data_pagamento LIKE ?", args: [mesStr + '%'] }),
                ...
                client.execute({ sql: "SELECT p.id, p.nome, p.telefone, MAX(a.data) as ultima_visita, CAST(julianday('now')-julianday(MAX(a.data)) AS INTEGER) as dias_ausente FROM pacientes p LEFT JOIN agendamentos a ON p.id=a.paciente_id GROUP BY p.id HAVING ultima_visita < date('now','-180 days') OR ultima_visita IS NULL ORDER BY ultima_visita ASC LIMIT 50", args: [] }),
                client.execute({ sql: "SELECT substr(data,1,7) as mes, COUNT(*) as total FROM agendamentos WHERE data >= date('now','-6 months') GROUP BY mes ORDER BY mes", args: [] }),
                client.execute({ sql: "SELECT categoria, COUNT(*) as total FROM agendamentos WHERE categoria IS NOT NULL AND categoria!='' GROUP BY categoria ORDER BY total DESC LIMIT 10", args: [] }),
            ])
            return res.status(200).json({
                ...
                resumo: {
                    ...
                    receita_total: rs[5].rows[0].total
                },
                profissionais: rs[6].rows,
                inativos: rs[7].rows,
                agendamentos_por_mes: rs[8].rows,
                agendamentos_por_categoria: rs[9].rows
            })
        }
        ...
        if (route === 'pacientes') {
            ...
            if (status === 'inativos') {
                ...
                sql = "SELECT p.id,p.nome,p.telefone,p.email,p.data_nascimento,MAX(a.data) as ultima_visita,CAST(julianday('now')-julianday(MAX(a.data)) AS INTEGER) as dias_ausente FROM pacientes p LEFT JOIN agendamentos a ON p.id=a.paciente_id" + where + " GROUP BY p.id ORDER BY p.nome ASC LIMIT ? OFFSET ?"
            }
            args.push(lim, off)
        ...
        if (route === 'agendamentos') {
            ...
            var sqlAg, argsAg = []
            var base = "SELECT a.id,a.data,a.hora_inicio,a.hora_fim,a.categoria,a.status,a.observacoes,a.profissional_id,p.nome as paciente_nome,p.telefone as paciente_telefone,pr.nome as profissional_nome FROM agendamentos a LEFT JOIN pacientes p ON p.id=a.paciente_id LEFT JOIN profissionais pr ON pr.id=a.profissional_id"

            if (q.dataInicio && q.dataFim) {
                sqlAg = base + " WHERE a.data BETWEEN ? AND ? ORDER BY a.data DESC,a.hora_inicio DESC LIMIT " + lim2
                argsAg = [q.dataInicio, q.dataFim]
            } else if (q.data) {
                sqlAg = base + " WHERE a.data=? ORDER BY a.hora_inicio"
                argsAg = [q.data]
            } else {
                var m = q.mes || new Date().toISOString().slice(0, 7)
                sqlAg = base + " WHERE a.data LIKE ? ORDER BY a.data DESC,a.hora_inicio DESC LIMIT " + lim2
                argsAg = [m + '%']
            }
        ...
        if (route === 'agenda-view') {
            ...
            var sqlAv, argsAv
            if (q.profissional) {
                sqlAv = "SELECT a.*,p.nome as paciente_nome,p.telefone as paciente_telefone,pr.nome as profissional_nome FROM agendamentos a LEFT JOIN pacientes p ON p.id=a.paciente_id LEFT JOIN profissionais pr ON pr.id=a.profissional_id WHERE a.data=? AND a.profissional_id=? ORDER BY a.hora_inicio"
                argsAv = [d, q.profissional]
            } else {
                sqlAv = "SELECT a.*,p.nome as paciente_nome,p.telefone as paciente_telefone,pr.nome as profissional_nome FROM agendamentos a LEFT JOIN pacientes p ON p.id=a.paciente_id LEFT JOIN profissionais pr ON pr.id=a.profissional_id WHERE a.data=? ORDER BY pr.nome,a.hora_inicio"
                argsAv = [d]
            }
            var rav = await client.execute({ sql: sqlAv, args: argsAv })
            var profs3 = await client.execute('SELECT id,nome,especialidade,cor_agenda FROM profissionais WHERE ativo=1 ORDER BY nome')
        ...
        if (route === 'financeiro') {
            ...
            if (!q.de && !q.ate) {
                ...
                fw.push('f.data_pagamento LIKE ?'); fa.push(m2 + '%')
            }
            ...
            var ft = await client.execute({ sql: "SELECT COALESCE(SUM(CASE WHEN tipo='entrada' THEN valor ELSE 0 END),0) as total_entradas,COALESCE(SUM(CASE WHEN tipo='saida' THEN valor ELSE 0 END),0) as total_saidas,COUNT(*) as total_registros FROM financeiro f" + fwc, args: fa })
        ...
        if (route === 'crc') {
            ...
            var rc = await client.execute({ sql: "SELECT p.id,p.nome,p.telefone,p.email,MAX(a.data) as ultima_visita,CAST(julianday('now')-julianday(MAX(a.data)) AS INTEGER) as dias_ausente FROM pacientes p LEFT JOIN agendamentos a ON p.id=a.paciente_id" + cw + having + " ORDER BY dias_ausente DESC LIMIT 200", args: ca })
        ...
        if (route === 'relatorios') {
            ...
            if (tipo === 'producao') {
                var prod = await client.execute({ sql: "SELECT substr(data,1,7) as mes,COUNT(*) as agendamentos FROM agendamentos WHERE data >= date('now','-' || ? || ' months') GROUP BY mes ORDER BY mes", args: [meses2] })
                var rece = await client.execute({ sql: "SELECT substr(data_pagamento,1,7) as mes,SUM(valor) as receita FROM financeiro WHERE data_pagamento >= date('now','-' || ? || ' months') AND tipo='entrada' GROUP BY mes ORDER BY mes", args: [meses2] })
            ...
            if (tipo === 'procedimentos') {
                var procs = await client.execute("SELECT categoria,COUNT(*) as total FROM agendamentos WHERE categoria IS NOT NULL AND categoria!='' GROUP BY categoria ORDER BY total DESC LIMIT 15")
        ...
        if (route === 'busca') {
            ...
            var ba = await client.execute({ sql: "SELECT a.id,a.data,a.hora_inicio,a.categoria,a.status,p.nome as paciente_nome,pr.nome as profissional_nome FROM agendamentos a LEFT JOIN pacientes p ON p.id=a.paciente_id LEFT JOIN profissionais pr ON pr.id=a.profissional_id WHERE p.nome LIKE ? ORDER BY a.data DESC LIMIT 10", args: [bt] })
        ...
        if (route === 'conta-corrente') {
            ...
            ccr.rows.forEach(function(r){ if (r.tipo==='entrada') totalE += (r.valor||0); else totalS += (r.valor||0) })
        ...
        if (route === 'metas') {
            ...
            var mpr = await client.execute({ sql: "SELECT pr.*,COUNT(a.id) as agendamentos_mes FROM profissionais pr LEFT JOIN agendamentos a ON pr.id=a.profissional_id AND a.data LIKE ? AND a.status!='cancelado' GROUP BY pr.id ORDER BY agendamentos_mes DESC", args: [mstr + '%'] })
            var mtm = await client.execute({ sql: "SELECT COUNT(*) as total FROM agendamentos WHERE data LIKE ? AND status!='cancelado'", args: [mstr + '%'] })
            ...
            var metas = await client.execute({ sql: "SELECT * FROM metas WHERE mes=? AND ano=?", args: [mm, ma] })
            return res.status(200).json({ success: true, mes: mm, ano: ma, resumo_mes: { agendamentos: mtm.rows[0].total, receita: mrm.rows[0].total }, profissionais: mpr.rows, metas: metas.rows })
        }
